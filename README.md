...
# 專案簡介與開發動機
TraceLine 是一款基於 前後端分離 (Decoupled Architecture) 理念開發的 SPA 單頁式計畫管理平台。

開發動機源於對現代碎片化任務管理的思考：許多工具僅停留在代辦清單層面，缺乏「目標脈絡」。本專案透過 「主-子計畫」的層級架構，協助使用者從主要目標（主計畫）深入到各個階段的步驟（子計畫），並透過自動化的進度加權計算，掌握專案整體進度。
...
進度連動邏輯: 子計畫狀態改變時，後端自動重新加權計算主計畫之完成率，確保數據即時準確。
...
API 效能優化: 在特定情境使用 Dapper 增加效能運用，替代 EF Core，提升查詢效率。
...
解決: 運用 視覺層級 (Visual Hierarchy) 概念，淡化次要日期資訊並強化核心進度數字，達成兼具資訊量與美感的平衡。
